...
def best_score(kv, b_obj):
    subreddit = kv[0]
    average = b_obj.value[subreddit]
    score = kv[1]['score']
    author = kv[1]['author']
...
def main(inputs, output):
    ...
    bcast = dict(averages.collect())
    b_obj = sc.broadcast(bcast)
    best_scores = commentbysub.map(lambda x:best_score(x, b_obj))
